...
def generic(opts, mysql_connection)
  ...
  Dir.glob("#{dir}/**/*.pdf") do |d|
    ...
    year = File.dirname(d).split("/").last
    puts year
    name = File.basename(d).split("_")
    month=name[0]
    date = name[1]
    edition="01"
    puts month
    ...
    temp_dir = "upload_gtw1"
    object=year+month+date
    temp_location = File.join(temp_dir, object)
    puts temp_location
    puts "---------------------------------"
    target_dir = File.join(temp_location,"PDF")
    if not File.directory?(temp_location)
      FileUtils::mkdir_p target_dir
      files = Dir.glob(object_path+"/#{month}_#{date}*.pdf")
      ...
      puts insert
      Utils.tar(File.join(temp_location, "pdf.tar"), "#{target_dir}")
      #delete untar file
      FileUtils.rm_rf(target_dir)
      #create md5 for each file in a folder
