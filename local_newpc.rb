...
def peel(opts, mysql_connection)
  ...
  Dir.glob("#{dir}/**/*.jp2") do |d|
    puts d
    item = File.basename(d).split(".").first[0,8]
    puts item
    item_path=dir
    pagecount = Dir.glob("#{item_path}/**/#{item}*.jp2").count
    ...
    puts temp_location
    target_dir = File.join(temp_location,"JP2")
    FileUtils::mkdir_p target_dir
    files = Dir.glob(item_path+"/**/#{item}*.jp2")
    create_bag(target_dir, files, false)
    Utils.tar(File.join(temp_location, "jp2.tar"), "#{target_dir}")
    #delete untar file
    FileUtils.rm_rf(target_dir)
    #create md5 for each file in a folder
