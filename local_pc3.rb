...
def peel(opts, mysql_connection)
  ...
  Dir.glob("#{dir}/**/*.tif") do |d|
    ...
    puts object_path
    name = File.basename(d).split(".")[0][0,8]
    puts name



    temp_dir = "upload_pc"
    temp_location = File.join(temp_dir, name)
    puts temp_location
    puts "---------------------------------"
    target_dir = File.join(temp_location,"TIF")
    if not File.directory?(temp_location)
      FileUtils::mkdir_p target_dir
      files = Dir.glob(object_path+"/#{name}*.tif")
      pagecount=files.count
      create_bag(target_dir, files, false)
      insert = "INSERT INTO items(code, digstatus, delivery, scanimages) VALUES ('#{name}', 'digitized', '#{delivery}', '#{pagecount}') ON DUPLICATE KEY UPDATE code = VALUES(code), digstatus=VALUES(digstatus), delivery=VALUES(delivery), scanimages=VALUES(scanimages)"
      puts insert
